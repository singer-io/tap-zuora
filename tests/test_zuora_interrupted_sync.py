    ...
    def run_test(self, api_type):
        ...
        for catalog in found_catalogs:
            ...
            connections.select_catalog_and_fields_via_metadata(conn_id, catalog, 
                                                               annoted_schema, additional_md=additional_md, 
        ...
        for stream in expected_streams:
            with self.subTest(stream=stream):
                ...
                LOGGER.info("first_bookmark_value =%s \n second_bookmark_value =%s", first_bookmark_value, \
                ...
                if expected_replication_method == self.INCREMENTAL:
                    if stream in completed_streams:
                        ...
                        self.assertGreaterEqual(second_sync_count,
                                                1, 
                                                msg="Incorrect bookmarking for {0}, at least one or more record \
                                                should be replicated".format(stream))
                        ...
                        self.assertGreaterEqual(second_sync_count,
                                                first_sync_count,
                                                msg="For pending sync streams - {0}, second sync record count should \
                                                be more than or equal to first sync".format(stream))  

                    else:
                        raise Exception("Invalid state of stream {0} in interrupted state, \
                                         please update appropriate state for the stream".format(stream))
