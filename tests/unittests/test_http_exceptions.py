import unittest
from tap_zuora.exceptions import RateLimitException, RetryableException, BadCredentialsException
import requests
import tap_zuora
...
class MockConfigAqua:
    ...
    """
    config = {'start_date': '', "username": '', 'password': '', 'api_type': 'AQUA', 'partner_id': 'salesforce'}
...
class TestHttpExceptionErrors(unittest.TestCase):
    ...
    def test_http_429_error(self, mock_time, mock_http_request, mock_http_send):
        """
        Test if API request gets retried for 5 times after encountering ratelimit exception
        """
        ...
        self.assertEquals(mock_http_send.call_count, 5)
        self.assertEquals(mock_http_request.call_count, 5)

    def test_http_5xx_error(self, mock_time, mock_http_request, mock_http_send):
        """
        Test if API request gets retried for 5 times after encountering 500, 502, 503, 504 exceptions
        """
        client_object = Client.from_config(MockConfigAqua.config)
        mock_http_request.return_value = requests.Request()
...
class TestGetUrlScenarios(unittest.TestCase):
    ...
    def test_bad_credentials_aqua(self, mock_args):
        """
        Test for BadCredentialsException for incorrect credentials for AQuA API calls
        """
    ...
    def test_bad_credentials_rest(self, mock_args):
        """
        Test for BadCredentialsException for incorrect credentials for REST API calls
        """
